...
move_t best_move_kayles(int coins[], int num_coin)
{
	move_t p;
	int l, n, i;
	...
	for (i = 0; i < num_coin; i++) {
		if (coins[i] == 1) {
			coins[i] = 0;
			p = best_move_kayles(coins, num_coin);
			coins[i] = 1;

			if (p.win == 0)
				return memo[n] = (move_t) {1, i, 1};
		}
...
int main()
{
	...
	while (1) {
		...
		my_move = best_move_kayles(coins, n);
		if (my_move.win == 1)
			printf("I will win!\n");
